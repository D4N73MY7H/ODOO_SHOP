...
def generate_pdf(info, extra):
    ...
    report_info = Paragraph(f"<font size=10>Report Period: {info.get('date')} to {info.get('till')} - Employee: {info.get('name')} (ID: {info.get('id')})</font>", styles['Normal'])
    date_info = Paragraph(f"<font size=10>Date: {current_date}</font>", styles['Normal'])
