...
class Payments(models.Model):
    ...
    _sql_constraints = [
        ('check_amount_positive', 'CHECK(amount > 0)', 'The amount of payment must be greater than zero.')
    ]
    ...
    def _check_amount_positive(self):
        for payment in self:
            if payment.amount <= 0.0:
                raise ValidationError("The amount of payment must be greater than zero.")
